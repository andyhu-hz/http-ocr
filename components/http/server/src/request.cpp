...
#include <cstdlib>
#include "log.h"
const uint32_t MAX_RECV_BUFFER_SIZE = 8192;

namespace timax
{

	request::request()
		:buffer_{static_cast<char*>(std::malloc(MAX_RECV_BUFFER_SIZE)), 0, MAX_RECV_BUFFER_SIZE}
	{
	...
	bool request::parse_form_multipart()
	{
		if (!multipart_parser_)
		{
			...
			//std::cout << "request::parse_form_multipart, content_type =" << content_type << std::endl;
			...
			//cout << "request::parse_form_multipart, boundary = " << boundary << endl;
